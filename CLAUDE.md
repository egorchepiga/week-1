...
└── Шаг 5: Финальный выбор ТОП-50 (НЕ ТОП-3!)
    └── ⚠️ ВАЖНО: Выбирать 50 идей для Keyword Research

LESSON 02: Keyword Research (для всех ТОП-50!)
├── Шаг 1: Сбор данных Semrush для каждой из 50 идей
...
└── Шаг 5: Финальное решение GO/NO-GO для каждой идеи
    └── ⚠️ Все 50 идей должны быть проанализированы!
...
## 🎯 ВАЖНО: Выбор ТОП-50 идей (не ТОП-3!)

> **Правило:** При выполнении уроков 1-2 всегда выбирать и анализировать **50 идей**, а не 3.

### Почему ТОП-50?

1. **Максимальный охват** — больше шансов найти идеальную нишу
2. **Запас на ошибки** — многие ключи окажутся занятыми
3. **Диверсификация** — можно запустить несколько расширений параллельно
4. **Глубокое понимание рынка** — видишь паттерны и тренды
5. **Выявление GAP-ов** — находишь незанятые ниши

### Алгоритм выбора ТОП-50

```
1. Сгенерировать 100+ идей
   └── MongoDB + SUMMARY.md + курс + @chrome-extensions-analyst

2. Отсеять по RED FLAGS
   └── ERRORS_CATALOG.md (обычно -20-30 идей)

3. Оценить оставшиеся по 8 критериям
   └── Баллы 0-50

4. Выбрать ТОП-50 по баллам
   └── Записать в FINAL_IDEAS_REPORT.md

5. Для КАЖДОЙ из 50 провести Keyword Research
...
```
lesson-01/outputs/
├── IDEAS_SCREENING.md       # 100+ идей → фильтр → 70+
├── IDEAS_ANALYSIS.md        # Оценка 70+ по 8 критериям
└── FINAL_IDEAS_REPORT.md    # ТОП-50 для Keyword Research
...
lesson-02/outputs/
└── KEYWORD_RESEARCH_REPORT.md  # Финальный рейтинг всех 50
```
